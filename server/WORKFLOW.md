...
| A binary patch to the client `.so` | `rebuild_arm64.py` (v170) or `builders/build_private.py` (v171+). Offsets are per-version - re-derive from that version's own `dump.cs`, never shift the other set. |
...
| The build/patch/sign/install pipeline itself | `rebuild_arm64.py` (v170), `builders/build_private.py` (v171+ side-by-side King Bug Castle) |
