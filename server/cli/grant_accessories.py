...
"""
import argparse, datetime, math, xml.etree.ElementTree as ET
...
def load_admin_config(path):
    ...
    if not isinstance(rows, list):
        raise SystemExit("admin_accessories.json: accessories must be a list")
    legal = allowed_mains()
    units = per_score()
    ...
    for row in rows:
        ...
        rarity = int(row.get("rarity", RARITY))
        level = int(row.get("level", LEVEL))
        if rarity not in (1, 2, 3):
            raise SystemExit(f"admin_accessories.json: rarity {rarity} is not 1..3")
        if not 1 <= level <= 20:
            raise SystemExit(f"admin_accessories.json: level {level} is not 1..20")

        sub_rows = row.get("subStats") or []
        slots, budget = level_events(rarity)
        if not isinstance(sub_rows, list) or not sub_rows:
            raise SystemExit("admin_accessories.json: at least one sub-stat is required")
        if len(sub_rows) > slots:
            raise SystemExit(f"admin_accessories.json: rarity {rarity} allows {slots} sub-stat(s)")
        ...
        for s in sub_rows:
            if not isinstance(s, dict):
                raise SystemExit("admin_accessories.json: each sub-stat must be an object")
            try:
                key = s["key"]
                score = float(s["value"])
            except (KeyError, TypeError, ValueError) as e:
                raise SystemExit(f"admin_accessories.json: invalid sub-stat: {e}")
            if key not in units:
                raise SystemExit(f"admin_accessories.json: unknown sub-stat key {key!r}")
            if key in subs:
                raise SystemExit(f"admin_accessories.json: duplicate sub-stat key {key!r}")
            if not math.isfinite(score) or not 0 <= score <= (1000.0 if mega else 26.0):
                raise SystemExit(f"admin_accessories.json: invalid score {score} for {key}")
            subs.append(key)
        ...
        if not mega and sum(scores) > budget + 1e-6:
            raise SystemExit(f"admin_accessories.json: sub-stat scores exceed rarity {rarity} budget {budget}")

        out.append({
            "id": next_id, "accountId": 1, "unitId": 0, "slot": 0,
            "type": typ, "rarity": rarity,
            "level": level, "exp": 0,
            "synergy": syn, "state": 0,
            "data": {"mainStat": main, "subStats": rolls},
