...
## Seasonal Arena / Strife rankings

These boards use a separate service but the same authenticated game session. The
official API only issues `accesstoken`s to requests with a valid XIGNCODE cookie
(generated by the native SDK inside the real client) - it cannot be scripted.
Harvest a token from a genuine client login instead:

```bash
mitmdump --listen-host 0.0.0.0 -p 8888 -s api/auth/token_harvester.py
# point the device/redroid at the proxy (system CA), tap Guest login once
# -> captured_token.txt is written automatically

python3 api/ranking/fetch_seasonal.py   # token auto-loaded from captured_token.txt
```

`api/auth/guest_login.py` can script everything else (account creation, id
reuse, seed fetch) - only the final token step needs the real client.

The command reads `season` from `/pvp/info` and `/colosseum`, then requests:
`/ranking/pvp-ranking?season=...` and
`/ranking/colosseum-ranking?season=...` from
`https://kgc-ranking-1.awesomepiece.com`. Pass `--pvp-season N` or
`--colosseum-season N` for an archived season. `KGC_VERSION` must match the
installed client; the bundled v172.0.01 is the default.
